...
later pod saw /runpod-volume). The fix:
  - everything goes under one root you pass as --volume (e.g. /workspace)
...
Usage on pod:
  python -m extract.populate_data --volume /workspace          # download everything
  python -m extract.populate_data --volume /workspace --verify  # just re-check manifest
...
def populate(args):
    ...
    for name, hint in [
        ("cifar10c", "CIFAR-10-C: bash pod_atlas.sh does it (zenodo CIFAR-10-C.tar, extracted FLAT: <this dir>/<corruption>.npy)"),
...
if __name__ == "__main__":
    ...
    ap.add_argument("--volume", default="/workspace",
                    help="volume mount root on THIS pod (pods: /workspace; serverless: /runpod-volume)")
