...
Seeds 3 and 4 (Stage 1b, docs/plans/STAGE1.md amendment 2), Stage 2 (resnet56, docs/plans/STAGE2.md) and row 9
(A3 margin/type-b, docs/plans/A3_MARGIN.md) are pre-registered for one pod session (results/atlas_v1_resnet20_s3/RUN_REQUEST.md).
...
| 9 | penult | margin_typeb (type-b = wrong and maxprob > 0.7) | top-2 margin to train-reference centers separates correct from type-b test samples better than nearest-center distance (M2); type-b sits on ridges, median margin ≤ 0.5 × correct (M3) | ⬜ | legacy 0.895 / 0.763 (docs/history/SESSION_RESULTS_VALLEYS_TYPEB_SCALE.md:55-59; Upgraded-Mod sample_and_scale.py, test-set centers, full 10k, std (0.2470, 0.2435, 0.2616)); manifests margin_* (pre-registration experiments/queue/margin_v1_resnet20_s1.yaml) |
