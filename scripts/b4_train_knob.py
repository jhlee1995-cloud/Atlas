"""
b4_train_knob.py -- batch-4 knob lane (docs/plans/B4_INTEGRATION.md D13): a VERBATIM copy of scripts/train_second_seed.py
(frozen since 2b561a9) with ONE added argument, --label-smoothing, passed to nn.CrossEntropyLoss(label_smoothing=...).
Default 0.0 reproduces the frozen recipe exactly. The diff against train_second_seed.py is reviewed at P1
(git diff --no-index scripts/train_second_seed.py scripts/b4_train_knob.py).
...
def main():
    ...
    ap.add_argument("--label-smoothing", type=float, default=0.0, help="batch-4 knob (D13); 0.0 = the frozen recipe")
