// B1b verdicts (docs/plans/B1B_AMENDMENT.md): scripts/b1_verdicts.js with exactly three changes, marked "B1b:" --
// (1) the G0 legacy valley-separation band, (2) the gate record read from results/b1b_gate, (3) provenance in two parts
// (the ResNet50 runs against B1's P and P_run list; the ViT runs against P_B = the B1b commit). Everything else is B1's rule.
//   node scripts/b1b_verdicts.js --p f1c3c43 --p-run d257d91,68244f7 --p-b <P_B> --p-b-run <P_B_run> [--a4b ...] [--json ...]
...
const PB_ARG = arg("--p-b"), PBRUN_ARG = arg("--p-b-run");   // B1b: the ViT runs' commits
...
const LEGACY_N = [10048, 10111], LEGACY_ACC = [0.766, 0.776], LEGACY_SEP = [1.2481, 1.2881];   // B1b: was [1.12, 1.16]
...
const gateRec = loadJ(R("b1b_gate", "gate.json"));   // B1b: was b1_gate
...
function provenanceFor(P_ARG, PRUN_ARG, names, frozen) {   // B1b: parameterised (was provenance())
...
const RN50 = ["resnet50_legacy10k", "resnet50", "resnet50_swap", ...E9];
const VITRUNS = ["vitb16", "vitb16_swap", "deitb", "deitb_swap"];
const FROZEN_B = [...FROZEN, "docs/plans/B1B_AMENDMENT.md", "scripts/b1b_gate.py", "scripts/b1b_verdicts.js"];
function provenance() {   // B1b: ResNet50 + E9 runs against B1's P / P_run; ViT runs against P_B / P_B_run
  const a = provenanceFor(P_ARG, PRUN_ARG, RN50.flatMap(x => [x, `${x}_r2`]).map(x => `margin_b1_${x}`), FROZEN);
  const b = provenanceFor(PB_ARG, PBRUN_ARG, VITRUNS.flatMap(x => [x, `${x}_r2`]).map(x => `margin_b1_${x}`), FROZEN_B);
  const probs = [...(a.problems || []).map(x => `B1 part: ${x}`), ...(b.problems || []).map(x => `B1b part: ${x}`)];
  return { b1_part: a, b1b_part: b, status: a.status === "PASS" && b.status === "PASS" ? "PASS" : (probs.length ? "FAIL" : "NOT_EVALUABLE"), problems: probs };
}
